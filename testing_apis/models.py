...
from insuretech_apis import keygenv1 as kv

class InsuranceUserProfile (models.Model):
...
@receiver(post_save, sender=InsuranceUserProfile)
...
@receiver(post_save, sender=InsuranceUserProfile)
def save_password(sender, instance, **kwargs):
    instance.password = instance.fobj.encrypt(instance.password)
    instance.password.save()

#post_save.connect(save_profile, sender=InsuranceUserProfile)

class UserbankDetails (models.Model):
    insuranceuserprofile = models.ForeignKey(InsuranceUserProfile, on_delete=models.CASCADE)
...
class UserworkDetails (models.Model):
    insuranceuserprofile = models.ForeignKey(InsuranceUserProfile, on_delete=models.CASCADE)
